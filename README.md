...
Node and edge data are stored in `nodes.csv` and `edges.csv` with safe file-locking to prevent corruption.
...
- **Route Finder**  
  ...
  - Or use browser GPS for "Use my location" start point.
...
- **No GPS Tracking**  
  - Location is not stored and only used for the routing.

### **Admin Interface**
Accessible via `/wayfinding/add_node` after login.

- **Add Nodes**
  - Click anywhere on the map, assign a label, save to `nodes.csv`.
  - Auto-suggests the next available connector label (`c###`).
- **Add Edges**
  - Click two existing nodes; server computes the distance.
  - Saves to `edges.csv` after deduplication.
- **Delete Edges**
  - Click an edge to highlight, then delete in one step.
...
```
.
├── app.py                      # Main Flask app with routing + admin UI
...
### **Systemd Service**
`/etc/systemd/system/wayfinding.service`: (partial snapshot)
